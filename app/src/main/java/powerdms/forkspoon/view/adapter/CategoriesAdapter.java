...
public class CategoriesAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements Filterable {
    ...
    public List<Category> filtered_items = new ArrayList<>();
    private int visibleThreshold = 10;
    private int lastVisibleItem, totalItemCount;
    public OnItemClickListener mListener;
    public OnLoadMoreListener mOnLoadMoreListener;

    public CategoriesAdapter(Context context, RecyclerView recyclerView, OnItemClickListener onItemClickListener) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof CategoryVH) {
            CategoryVH viewHolder = (CategoryVH) holder;
    ...
    public class ItemFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            for (int i = 0; i < list.size(); i++) {
                String str_name = list.get(i).getCategories().getName();
